...
XPK_COMMAND_128="git clone -b ${BRANCH_NAME} https://github.com/${GITHUB_USER}/accelerator-microbenchmarks.git && cd accelerator-microbenchmarks && pip install -r requirements.txt && python src/run_benchmark.py --config=${CONFIG_128} && gsutil -m cp /tmp/microbenchmarks/outputs/metrics_report.jsonl ${GCS_PATH_128}"
...
XPK_COMMAND_256="git clone -b ${BRANCH_NAME} https://github.com/${GITHUB_USER}/accelerator-microbenchmarks.git && cd accelerator-microbenchmarks && pip install -r requirements.txt && python src/run_benchmark.py --config=${CONFIG_256} && gsutil -m cp /tmp/microbenchmarks/outputs/metrics_report.jsonl ${GCS_PATH_256}"
